...
const deliverySum = JSON.parse(fs.readFileSync(path.join(process.cwd(), "src", "json", "delivery.json"), "utf-8"));
...
indexRouter.get("/delivery", (req, res) => {
    try {
        res.send({
            status: 200,
...
indexRouter.put("/delivery", (req, res) => {
    try {
        ...
        fs.writeFileSync(path.join(process.cwd(), "src", "json", "delivery.json"), JSON.stringify(deliverySum));
    }
    catch (error) {
        res.send({
            message: `Error: ${error.message}`,
...
indexRouter.get(`/ShopData`, (req, res) => {
    try {
        res.send({
            status: 200,
            message: `Shop Datas`,
...
indexRouter.put('/ShopData', (req, res) => {
    try {
