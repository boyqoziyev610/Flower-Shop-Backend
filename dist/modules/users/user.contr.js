...
export class UserContr {
    ...
    static async AddEmployer(req, res) {
        try {
            let { username, email, website, job, phone, img_link } = req.body;
    ...
    static async ForgetPassword(req, res) {
        try {
            ...
            let updated = await userSchema.findByIdAndUpdate(checkCorrect._id, { password: sha256(password) }, { new: true });
            delete updated?.role;
            res.send({
                status: 200,
    ...
    static async UpdateMyProfile(req, res) {
        try {
            ...
            let updatedProfile = await userSchema.findByIdAndUpdate(user_id, { username, email, website, phone, img_link }, { new: true });
    ...
    static async UpdateEmployer(req, res) {
        try {
            ...
            let updatedEmployer = await userSchema.findByIdAndUpdate(id, { username, password, role, email, website, job, phone }, { new: true });
