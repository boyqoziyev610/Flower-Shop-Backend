...
export class FlowersContr {
    constructor() { }
    static async GetFlowers(req, res) {
        try {
            let { id } = req.params;
    ...
    static async AddFlower(req, res) {
        try {
    ...
    static async PutFlower(req, res) {
        try {
            let { id } = req.params;
            ...
            let updatedFlower = await flowerSchema.findByIdAndUpdate(id, { name, img_link, price, category, desc, solded, type_f }, { new: true });
            res.send({
                status: 200,
    ...
    static async DeleteFlower(req, res) {
        try {
            let { id } = req.params;
            let checkExists = await flowerSchema.findById(id);
            if (checkExists == null) {
